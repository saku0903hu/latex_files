...
\usepackage{amsmath, amsfonts, bm, mathtools} % 数学パッケージ
\usepackage{luatexja} % 日本語サポート
\usepackage{diagbox} % 斜線付きセルを作成するためのパッケージ

% プリアンブルに追加
\usepackage{tcolorbox} % tcolorboxパッケージの読み込み

% 定義環境のカスタム設定
\tcolorboxenvironment{def}{
  colback=blue!5!white, % 背景色
  colframe=blue!75!black, % 枠線の色
  fonttitle=\bfseries, % タイトルのフォント設定
  title=Def. % タイトルを"Def."に設定
}
